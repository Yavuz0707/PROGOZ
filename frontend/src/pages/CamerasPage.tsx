...
export default function CamerasPage() {
  ...
  const [plateFrameInterval, setPlateFrameInterval] = useState(10);
  ...
  async function submit(event: FormEvent) {
    ...
    await unwrap<Camera>(api.post("/cameras", { name, source_type: sourceType, rtsp_url: sourceType === "rtsp" ? rtspUrl : null, location: "Demo", plate_recognition_enabled: plateRecognitionEnabled, plate_frame_interval: plateFrameInterval }));
    load();
  }

  async function updatePlateSettings(camera: Camera, enabled: boolean, interval: number) {
    await unwrap<Camera>(api.put(`/cameras/${camera.id}`, { plate_recognition_enabled: enabled, plate_frame_interval: interval }));
    load();
  }
  ...
  return (
    <section className="space-y-5">
      ...
      <form onSubmit={submit} className="panel grid gap-3 p-4 md:grid-cols-[1fr_150px_1fr_170px_130px_auto]">
        ...
        <label className="flex items-center gap-2 text-sm text-slate-300"><input type="checkbox" checked={plateRecognitionEnabled} onChange={(e) => setPlateRecognitionEnabled(e.target.checked)} /> Plaka tanima</label>
